...
for file in ${IN_PATH}/*_R1_001.fastq
do
	sample_name="$(basename -s _R1_001.fastq ${file})"
	echo ${sample_name}
	if [ ${MODE} = 'single' ]; then
		...
		if [ ${STRANDNESS} = 'forward' ]; then
		    ...
		    ${HISAT2_PATH} --rna-strandness FR -p 20 -x ${HISAT_INDEX}/genome  -1 ${file} -2 ${IN_PATH}/${sample_name}_R2_001.fastq \
		    -S ${out_folder}/${sample_name}.sam  --summary-file ${out_folder}/${sample_name}.hisat
		elif [ ${STRANDNESS} = 'reverse' ]; then
			echo 'reverse-stranded'
			${HISAT2_PATH} --rna-strandness RF -p 20 -x ${HISAT_INDEX}/genome  -1 ${file} -2 ${IN_PATH}/${sample_name}_R2_001.fastq \
		    -S ${out_folder}/${sample_name}.sam  --summary-file ${out_folder}/${sample_name}.hisat
		elif [ ${STRANDNESS} = 'non-stranded' ]; then
			echo 'non-stranded'
			${HISAT2_PATH} -p 20 -x ${HISAT_INDEX}/genome  -1 ${file} -2 ${IN_PATH}/${sample_name}_R2_001.fastq \
			-S ${out_folder}/${sample_name}.sam  --summary-file ${out_folder}/${sample_name}.hisat
		fi
	fi
done
